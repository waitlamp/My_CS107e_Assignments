...
> __Tip__ Our reference version of the ps2 module supports reading in either polling
mode or by interrupts. A device defaults to polling and can be switched into interrupt
mode by calling the function `ps2_use_interrupts()`. Your ps2 module does not
...
- Inner loops are the first place to look for speed-up opportunities (e.g. hoisting
  ...
  saved.
- Every call to `gl_draw_char` calls `font_get_glyph` to re-extract the character
...
file  Use `make run` to build and run.  You can now program your Pi almost like
...
the header file
[armtimer.h](/header#armtimer)).  Attach the function `gprof_handler` as an interrupt handler. `gprof_handler` takes one
